...
public class Rule extends NavigationMixin{
   ...
   /**
     * Get a rule from the rule base (that is the ontology that contains the rules and the recipe).
     * curl -v -X GET http://localhost:8080/kres/rule/http://kres.iks-project.eu/ontology/meta/rmi.owl#ProvaParentRule
    ...
    public Response getRulesOfRecipe(@PathParam("uri") String recipeURI){
    	...
    	String recipeURIEnc;
		try {
			recipeURIEnc = URLEncoder.encode("http://kres.iks-project.eu/ontology/meta/rmi_config.owl#MyRecipeA", "UTF-8");
   ...
   /**
     ...
     * curl -v -X POST -F "recipe=http://kres.iks-project.eu/ontology/meta/rmi.owl%23ProvaParentRecipe" -F "rule=http://kres.iks-project.eu/ontology/meta/rmi.owl%23ProvaRuleNEW" -F "kres-syntax=body -> head" -F "description=prova di aggiunta regola" http://localhost:8080/kres/rule
     * @param recipe {A string contains the IRI of the recipe where to add the rule}
     * @param rule {A string contains the IRI of the rule to be added at the recipe}
     * @param kres_syntax {A string contains the body and the head of the kres rule. If not specified the rule is search in the Ontology otherwise is added as new.}
    ...
    public Response addRuleToRecipe(@FormParam(value="recipe") String recipe,@FormParam(value="rule") String rule,@FormParam(value="kres-syntax") String kres_syntax,@FormParam(value="description") String description){
...
//        return Response.ok().build();
   ...
   /**
     * To delete a rule from a recipe or from the ontology. If the recipe is not specified the rule is deleted from the ontology.
     * curl -v -X DELETE -G \
     * -d recipe="http://kres.iks-project.eu/ontology/meta/rmi.owl#ProvaParentRecipe" \
     * -d rule="http://kres.iks-project.eu/ontology/meta/rmi.owl#ProvaParentNewRule" \
     * http://localhost:port/kres/rule
    *
     * @Param rule {A string contains an IRI of the rule to be removed}
     * @param recipe {A string contains an IRI of the recipe where remove the rule}
     * @return Return: <br/>
    ...
    public Response removeRule(@QueryParam(value="rule") String rule,@QueryParam(value="recipe") String recipe){
