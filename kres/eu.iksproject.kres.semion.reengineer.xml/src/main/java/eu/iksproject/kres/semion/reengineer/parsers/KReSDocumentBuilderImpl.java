...
package eu.iksproject.kres.semion.reengineer.parsers;
 ...
 * @version $Id$
 */
public class KReSDocumentBuilderImpl extends DocumentBuilder
        implements JAXPConstants
{
    ...
    private KReSDOMParser kReSdomParser = null;
    ...
    KReSDocumentBuilderImpl(KReSDocumentBuilderFactoryImpl dbf, Hashtable dbfAttrs, Hashtable features)
        throws SAXNotRecognizedException, SAXNotSupportedException {
        this(dbf, dbfAttrs, features, false);
    }

    KReSDocumentBuilderImpl(KReSDocumentBuilderFactoryImpl dbf, Hashtable dbfAttrs, Hashtable features, boolean secureProcessing)
        throws SAXNotRecognizedException, SAXNotSupportedException
    {
        kReSdomParser = new KReSDOMParser();
        ...
        if (dbf.isValidating()) {
            ...
            fInitErrorHandler = kReSdomParser.getErrorHandler();
        }

        kReSdomParser.setFeature(VALIDATION_FEATURE, dbf.isValidating());

        // "namespaceAware" == SAX Namespaces feature
        kReSdomParser.setFeature(NAMESPACES_FEATURE, dbf.isNamespaceAware());

        // Set various parameters obtained from DocumentBuilderFactory
        kReSdomParser.setFeature(INCLUDE_IGNORABLE_WHITESPACE, 
                !dbf.isIgnoringElementContentWhitespace());
        kReSdomParser.setFeature(CREATE_ENTITY_REF_NODES_FEATURE,
                !dbf.isExpandEntityReferences());
        kReSdomParser.setFeature(INCLUDE_COMMENTS_FEATURE,
                !dbf.isIgnoringComments());
        kReSdomParser.setFeature(CREATE_CDATA_NODES_FEATURE,
                !dbf.isCoalescing());
        ...
        if (dbf.isXIncludeAware()) {
            kReSdomParser.setFeature(XINCLUDE_FEATURE, true);
        }
        
        // If the secure processing feature is on set a security manager.
        if (secureProcessing) {
            kReSdomParser.setProperty(SECURITY_MANAGER, new SecurityManager());
        }
        
        this.grammar = dbf.getSchema();
        if (grammar != null) {
            XMLParserConfiguration config = kReSdomParser.getXMLParserConfiguration();
            ...
            if (grammar instanceof XSGrammarPoolContainer) {
                ...
                entityHandler.setDTDHandler(kReSdomParser);
                kReSdomParser.setDTDSource(entityHandler);
            ...
            ((XMLDocumentSource)validatorComponent).setDocumentHandler(kReSdomParser);
            kReSdomParser.setDocumentSource((XMLDocumentSource) validatorComponent);
        ...
        // Initial EntityResolver
        fInitEntityResolver = kReSdomParser.getEntityResolver();
    }
    ...
    private void setFeatures(Hashtable features)
        ...
        if (features != null) {
            for (Enumeration e = features.keys(); e.hasMoreElements();) {
                ...
                kReSdomParser.setFeature(feature, value);
            }
        }
    }

    /**
     * Set any DocumentBuilderFactory attributes of our underlying kReSdomParser
     *
     * Note: code does not handle possible conflicts between kReSdomParser
        ...
        throws SAXNotSupportedException, SAXNotRecognizedException
    {
        ...
        for (Enumeration e = dbfAttrs.keys(); e.hasMoreElements();) {
            ...
            if (val instanceof Boolean) {
                // Assume feature
                kReSdomParser.setFeature(name, ((Boolean)val).booleanValue());
            } else {
                // Assume property
                if (JAXP_SCHEMA_LANGUAGE.equals(name)) {
                    ...
                    if ( W3C_XML_SCHEMA.equals(val) ) {
                        if( isValidating() ) {
                            kReSdomParser.setFeature(XMLSCHEMA_VALIDATION_FEATURE, true);
                            // this should allow us not to emit DTD errors, as expected by the 
                            // spec when schema validation is enabled
                            kReSdomParser.setProperty(JAXP_SCHEMA_LANGUAGE, W3C_XML_SCHEMA);
                        }
                    }
               		...
               		if( isValidating() ) {
						...
						if(value !=null && W3C_XML_SCHEMA.equals(value)){
            				kReSdomParser.setProperty(name, val);
						}else{
    ...
    public Document parse(InputSource is) throws SAXException, IOException {
        ...
        kReSdomParser.parse(is);
        return kReSdomParser.getDocument();
    }

    public boolean isNamespaceAware() {
        try {
            return kReSdomParser.getFeature(NAMESPACES_FEATURE);
        } 
    ...
    public boolean isValidating() {
        try {
            return kReSdomParser.getFeature(VALIDATION_FEATURE);
        } 
        catch (SAXException x) {
            throw new IllegalStateException(x.getMessage());
        }
    ...
    public boolean isXIncludeAware() {
        try {
            return kReSdomParser.getFeature(XINCLUDE_FEATURE);
        }
    ...
    public void setEntityResolver(EntityResolver er) {
        kReSdomParser.setEntityResolver(er);
    }

    public void setErrorHandler(ErrorHandler eh) {
        kReSdomParser.setErrorHandler(eh);
    }
    ...
    public void reset() {
        ...
        if (kReSdomParser.getErrorHandler() != fInitErrorHandler) {
            kReSdomParser.setErrorHandler(fInitErrorHandler);
        }
        /** Restore the initial entity resolver. **/
        if (kReSdomParser.getEntityResolver() != fInitEntityResolver) {
            kReSdomParser.setEntityResolver(fInitEntityResolver);
        }
    }

    // package private
    KReSDOMParser getDOMParser() {
        return kReSdomParser;
    }
