package org.apache.stanbol.enhancer.store;
...
/**
 * JCR Store for standalone Stanbol Enhancer
 *
...
public class JCRStore implements Store {
    ...
    private static final Logger log = LoggerFactory.getLogger("org.apache.stanbol.enhancer.store.JCRStore");

    private static final String ENHANCER_ROOT_NODE = "enhancer";
    ...
    public static Node findNodeById(String id, Node parent)
        ...
        if (id != null && id.length() > 0) {
            ...
            String queryString = "/jcr:root/" + parent.getPath() + "//*[@"
                    + JCRContentItem.ENHANCER_ID_PROP + "= '" + id + "']";
    ...
    private Node getParentNode() throws RepositoryException {
        ...
        if (rootNode.hasNode(ENHANCER_ROOT_NODE)) {
            return rootNode.getNode(ENHANCER_ROOT_NODE);
        } else {
            return rootNode.addNode(ENHANCER_ROOT_NODE);
        }
    }
