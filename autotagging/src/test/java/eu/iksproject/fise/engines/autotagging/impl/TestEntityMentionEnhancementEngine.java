...
import static eu.iksproject.fise.servicesapi.rdf.TechnicalClasses.FISE_ENTITYANNOTATION;
...
public class TestEntityMentionEnhancementEngine {
    ...
    public static void getTextAnnotation(ContentItem ci, String name, String context, UriRef type) {
        ...
        TextAnnotation testAnnotation = factory.getProxy(
                new UriRef("urn:iks-project:fise:test:text-annotation:person"), TextAnnotation.class);
    ...
    */
    ...
    private int checkAllEntityAnnotations(MGraph g) {
        Iterator<Triple> entityAnnotationIterator = g.filter(null, RDF_TYPE, FISE_ENTITYANNOTATION);
    ...
    private void checkEntityAnnotation(MGraph g, UriRef entityAnnotation) {
        ...
        while (relationToTextAnnotationIterator.hasNext()) {
            ...
            assertTrue(g.filter(referredTextAnnotation, RDF_TYPE, FISE_TEXTANNOTATION).hasNext());
        }

        // test if an entity is referred
        Iterator<Triple> entityReferenceIterator = g.filter(entityAnnotation, FISE_ENTITY_REFERENCE, null);
        ...
        Iterator<Triple> entityLabelIterator = g.filter(entityAnnotation, FISE_ENTITY_LABEL, null);
