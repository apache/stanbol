...
public class RefactorEnhancementEngine implements EnhancementEngine, ServiceProperties {
	...
	public int canEnhance(ContentItem ci) throws EngineException {
		 ...
		 * by other enhancement engines, as it must be the last engine in the chain.
		 * 
	...
	public void computeEnhancements(ContentItem ci) throws EngineException {
		...
		/*
		 * Now we prepare the OntoNet environment. First we create the OntoNet session
		...
		while (tripleIt.hasNext()) {
			...
			try {
				 ...
				 * The getEntityOntology is a method the do the job of asking
		...
		try {
			 ...
			 * returns the refactored graph expressed by using the given
			 * vocabulary.
			 */
			try {
	...
	protected void activate(ComponentContext context) {
        ...
        /*
         * The first thing to do is to create a recipe in the rule store that can be used
		...
		/* 
		 * The set of rule to put in the recipe can be provided by the user.
		 * A default set of rules is provided in /META-INF/default/seo_rules.sem.
		 * Use the property engine.refactor in the felix console to pass to the engine
		 * your set of rules.  
         * 
         */
		...
		if(recipeURI != null && !recipeURI.isEmpty()){
			...
			recipeStream = RefactorEnhancementEngine.class.getResourceAsStream("/META-INF/default/seo_rules.sem");
		}
	...
	/**
	 * Fetch the OWLOntology containing the graph associated to an entity from Linked Data.
	...
	private OWLOntology getEntityOntology(String entityURI){
		...
		if(entitySign != null){
			...
			RdfRepresentation entityRdfRepresentation = RdfValueFactory.getInstance().toRdfRepresentation(entityRepresentation);
		...
		if(entityMGraph != null){
			/*
			 * OWLOntologyManager manager = OWLManager.createOWLOntologyManager();
			 * final OWLOntology fetched = manager.loadOntologyFromOntologyDocument(dereferencer.resolve(entityReferenceString));
			 */
