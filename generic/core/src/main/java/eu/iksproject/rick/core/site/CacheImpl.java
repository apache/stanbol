package eu.iksproject.rick.core.site;
...
@Properties(value={
		...
		@Property(name=Cache.ADDITIONAL_MAPPINGS,cardinality=1000, //positive number to use an Array
			value={
				"| @=null;en;de;fr;it;es", //restrict languages
				"*" //cache all fields
			})
})
public class CacheImpl implements Cache {
	private Logger log = LoggerFactory.getLogger(CacheImpl.class);
	public static final String CACHE_FACTORY_NAME = "eu.iksproject.rick.yard.CacheFactory";
	...
	protected void activate(ComponentContext context) throws ConfigurationException,YardException,IllegalStateException, InvalidSyntaxException {
		...
		String cacheFilter = String.format("(&(%s=%s)(%s=%s))",
				...
				Yard.ID,yardId);
		yardTracker = new ServiceTracker(context.getBundleContext(),
...
//	}
	...
	@Override
	public Iterable<Representation> update(Iterable<Representation> represnetations) throws YardException, IllegalArgumentException {
		Yard yard = getCacheYard();
		if(yard == null){
			throw new YardException(String.format("The Yard %s for this cache is currently not available",yardId));
		} else {
			return yard.update(represnetations);
		}
	}
}
