package eu.iksproject.fise.jersey.resource;
...
import eu.iksproject.fise.jersey.cache.EntityCacheProvider;
...
public class StoreRootResource extends NavigationMixin {
    ...
    public StoreRootResource(@Context ServletContext context,
        ...
        // somewhere
        String q = "PREFIX fise: <http://fise.iks-project.eu/ontology/> "
        ...
        while (result.hasNext()) {
            ...
            if (ci != null) {
                ...
                Iterator<Triple> it = ci.getMetadata().filter(null,
                        Properties.FISE_EXTRACTED_FROM, content);
                while (it.hasNext()) {
                    it.next();
                    enhancements++;
                }
     ...
     *
     * @param localId the local id of the resource in the FISE store
     * @param headers
