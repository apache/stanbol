...
<pre>curl -X POST -d "context=http://dbpedia.org/resource/Paris&ldpath=name%20%3D%20rdfs%3Alabel%5B%40en%5D%3B" ${it.publicBaseUri}entityhub/ldpath</pre>
...
<form name="ldpathExample" id="ldpathExample">
    <input type="text" size="150" name="context">
...
function executeLDPath() {
 ...
 $.ajax({
   type: 'POST',
   url: "${it.publicBaseUri}entityhub/ldpath" ,
