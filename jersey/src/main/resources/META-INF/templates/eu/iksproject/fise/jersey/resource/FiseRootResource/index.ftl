...
<p>fise is an Open Source HTTP service meant to help Content Management System developers
to semi-automatically enhance unstructured content (text, image, ...) with semantic
annotations to be able to link documents with related entities and topics.</p>

<p>Please go to <a href="http://wiki.iks-project.eu/index.php/FISE">the official fise wiki</a>
to learn more on the project, read the documentation and join the mailing list.</p>

<p>Here are the main HTTP entry points. Each resource comes with a web view that documents
...
<dl>
  ...
  <dd>This is a <strong>stateless interface</strong> to allow clients to submit content to <strong>analyze</strong>
   by the fise <code>EnhancementEngine</code>s and get the resulting <strong>RDF enhancements</strong> at once
  ...
  <dd>
    ...
    <p>The console can also be used to perform hot-(re)deployment of any OSGi bundles.
       For instance to re-deploy a new version of this web interface, go to the <tt>$FISE_HOME/jersey</tt>
