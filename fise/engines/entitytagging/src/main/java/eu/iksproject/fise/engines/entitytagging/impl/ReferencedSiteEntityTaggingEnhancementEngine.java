...
public class ReferencedSiteEntityTaggingEnhancementEngine implements EnhancementEngine,
	...
	protected void activate(ComponentContext context) throws ConfigurationException {
		...
		this.personState = state == null ? true : Boolean.parseBoolean(state.toString());
		state = config.get(ORG_STATE);
		this.orgState = state == null ? true : Boolean.parseBoolean(state.toString());
		state = config.get(PLACE_STATE);
		this.placeState = state == null ? true : Boolean.parseBoolean(state.toString());
		Object type = config.get(PERSON_TYPE);
		this.personType = type == null || type.toString().isEmpty() ? null : NamespaceEnum.getFullName(type.toString());
		type = config.get(ORG_TYPE);
		this.orgType = type == null || type.toString().isEmpty() ? null : NamespaceEnum.getFullName(type.toString());
		type = config.get(PLACE_TYPE);
		this.placeType = type == null || type.toString().isEmpty() ? null : NamespaceEnum.getFullName(type.toString());
    ...
    protected final Iterable<Sign> computeEntityRecommentations(
        ...
        query.setConstraint(this.nameField, new TextConstraint(name.replace(' ', '+')));
        if(OntologicalClasses.DBPEDIA_PERSON.equals(type)){
        	if(this.personState){
        		if(this.personType!=null){
        	        query.setConstraint(Properties.RDF_TYPE.getUnicodeString(), new ReferenceConstraint(personType));
        		} // else no type constraint
        	} else { //ignore people
        		return Collections.emptyList();
        	}
        } else if(OntologicalClasses.DBPEDIA_ORGANISATION.equals(type)){
        	if(this.orgState){
        		if(this.orgType!=null){
        	        query.setConstraint(Properties.RDF_TYPE.getUnicodeString(), new ReferenceConstraint(orgType));
        		} // else no type constraint
        	} else { //ignore people
        		return Collections.emptyList();
        	}
        	...
        	if(this.placeState){
        		if(this.placeType!=null){
        	        query.setConstraint(Properties.RDF_TYPE.getUnicodeString(), new ReferenceConstraint(placeType));
        		} // else no type constraint
        	} else { //ignore people
        		return Collections.emptyList();
        	}
