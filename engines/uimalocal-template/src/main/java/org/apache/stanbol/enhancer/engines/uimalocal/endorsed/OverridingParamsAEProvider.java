package org.apache.stanbol.enhancer.engines.uimalocal.endorsed;
...
/**
 * {@link AEProvider} implementation that creates an Aggregate AE from the given
 * path, also injecting runtime parameters defined in the solrconfig.xml Solr
 * configuration file and assigning them as overriding parameters in the
 * aggregate AE
 *
 * @version $Id$
 */
public class OverridingParamsAEProvider implements AEProvider {
    ...
    public synchronized AnalysisEngine getAE() throws ResourceInitializationException {
        try {
            if (cachedAE == null) {
                ...
                XMLInputSource in = new XMLInputSource(new File(aeFilePath));
                ...
                //cachedAE = UIMAFramework.produceAnalysisEngine(desc,rscmgr,null);
                cachedAE = UIMAFramework.produceAnalysisEngine(desc);
                if (log.isDebugEnabled()) {
                    log.debug(new StringBuilder("AE ").append(cachedAE.getAnalysisEngineMetaData().getName()).append(" created from descriptor ").append(aeFilePath).toString());
                }
            } else {
                cachedAE.reconfigure();
                if (log.isDebugEnabled()) {
                    log.debug(new StringBuilder("AE ").append(cachedAE.getAnalysisEngineMetaData().getName()).append(" at path ").append(aeFilePath).append(" reconfigured ").toString());
                }
            }
        } catch (Exception e) {
            cachedAE = null;
    ...
    /*
     * create the value to inject in the runtime parameter depending on its
     * declared type
     */
