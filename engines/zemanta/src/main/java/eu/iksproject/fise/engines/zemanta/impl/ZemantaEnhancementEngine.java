...
import eu.iksproject.engines.zemanta.ZemantaOntologyEnum;
...
/**
 * FISE Zemanta enhancement engine.
...
public class ZemantaEnhancementEngine implements EnhancementEngine {
    ...
    @Property
    public static final String API_KEY_PROPERTY = "eu.iksproject.fise.engines.zemanta.key";	    
	...
	public void computeEnhancements(ContentItem ci) throws EngineException {
        ...
        //now we need to process the results and convert them into the FISE
			...
			if(target != null){
				...
				if(categorisationScheme != null && categorisationScheme.equals(ZemantaOntologyEnum.categorization_DMOZ.getUri())){
					...
					if(categoryTitle != null){
						//now write the FISE entity enhancement
						UriRef categoryEnhancement = EnhancementEngineHelper.createEntityEnhancement(enhancements, this, ciId);
						//write the title
						enhancements.add(new TripleImpl(categoryEnhancement, Properties.FISE_ENTITY_LABEL, literalFactory.createTypedLiteral(categoryTitle)));
						//write the reference
						if(categoryTitle.startsWith(ZEMANTA_DMOZ_PREFIX)){
							enhancements.add(new TripleImpl(categoryEnhancement, Properties.FISE_ENTITY_REFERENCE,
									new UriRef(DMOZ_BASE_URL+categoryTitle.substring(ZEMANTA_DMOZ_PREFIX.length()))));
						}
						//write the confidence
						if(confidence != null){
							enhancements.add(new TripleImpl(categoryEnhancement, Properties.FISE_CONFIDENCE, literalFactory.createTypedLiteral(confidence)));
						}
						...
						enhancements.add(new TripleImpl(categoryEnhancement,Properties.DC_TYPE, TechnicalClasses.FISE_CATEGORY));
						//Use the Zemanta Category as type for the referred Entity
						enhancements.add(new TripleImpl(categoryEnhancement, Properties.FISE_ENTITY_TYPE, ZemantaOntologyEnum.Category.getUri()));
					} else {
	 ...
	 * @param enhancements the graph containing the current FISE enhancements
	...
	protected void processRecognition(MGraph results, MGraph enhancements, String text, UriRef ciId){
		...
		while(recognitions.hasNext()){
			...
			log.info(" > confidence :"+confidence);
			String anchor = EnhancementEngineHelper.getString(
					results, recognition, ZemantaOntologyEnum.anchor.getUri());
			...
			//recognition
			NonLiteral object = EnhancementEngineHelper.getReference(
					results, recognition, ZemantaOntologyEnum.object.getUri());
			...
			for(Iterator<UriRef> sameAs = EnhancementEngineHelper.getReferences(
					results, object, ZemantaOntologyEnum.owlSameAs.getUri());
				sameAs.hasNext();
			...
			Iterator<UriRef> targets = EnhancementEngineHelper.getReferences(
					results, object, ZemantaOntologyEnum.target.getUri());
			String title = null;
			while(targets.hasNext()){
				//the entityRef is the URL of the target
				UriRef entity = targets.next();
				log.info("    -  target :"+entity);
				UriRef targetType = EnhancementEngineHelper.getReference(
						results, entity, ZemantaOntologyEnum.targetType.getUri());
				log.info("       o type :"+targetType);
				if(ZemantaOntologyEnum.targetType_RDF.getUri().equals(targetType)){
					String targetTitle = EnhancementEngineHelper.getString(results, 
							entity, ZemantaOntologyEnum.title.getUri());
			...
			for(NonLiteral relatedTextAnnotation : textAnnotations){
				enhancements.add(
						new TripleImpl(entityEnhancement, 
								...
								relatedTextAnnotation));
			}
			for(UriRef entity:sameAsSet){
				enhancements.add(new TripleImpl(entityEnhancement, Properties.FISE_ENTITY_REFERENCE, entity));
			}
			enhancements.add(new TripleImpl(entityEnhancement,Properties.FISE_ENTITY_LABEL,literalFactory.createTypedLiteral(title)));
		}
	}
	...
	private Collection<NonLiteral> processTextAnnotation(MGraph enhancements, String text, UriRef ciId, String anchor, Double confidence){
		...
		for(int current = text.indexOf(anchor);current >=0 ;current = text.indexOf(anchor,current+1)){
			...
			if(existingTextAnnotations != null){
				...
				//write the selection
				enhancements.add(new TripleImpl(textAnnotation, Properties.FISE_START, literalFactory.createTypedLiteral(current)));
				enhancements.add(new TripleImpl(textAnnotation, Properties.FISE_END, literalFactory.createTypedLiteral(current+anchorLength)));
				enhancements.add(new TripleImpl(textAnnotation, Properties.FISE_SELECTED_TEXT, anchorLiteral));
				...
				if(confidence != null){
					enhancements.add(new TripleImpl(textAnnotation, Properties.FISE_CONFIDENCE, literalFactory.createTypedLiteral(confidence)));
				}
	...
	private Map<Integer, Collection<NonLiteral>> searchExistingTextAnnotations(MGraph enhancements, Literal anchorLiteral) {
		Iterator<Triple> textAnnotationsIterator = enhancements.filter(null, Properties.FISE_SELECTED_TEXT, anchorLiteral);
		...
		while(textAnnotationsIterator.hasNext()){
			...
			//test rdfType
			if(enhancements.contains(new TripleImpl(subject, Properties.RDF_TYPE, TechnicalClasses.FISE_TEXTANNOTATION))){
				Integer start = EnhancementEngineHelper.get(enhancements, subject ,Properties.FISE_START, Integer.class, literalFactory);
				if(start != null){
